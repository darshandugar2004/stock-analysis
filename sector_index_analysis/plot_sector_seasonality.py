...
PLOTS_DIR = BASE_DIR / "selected_index_plots"
...
def prepare_yearly_returns(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    ...
    df["YearIndexedClose"] = (df["Close"] / first_close_by_year) * 100
    return df
...
def plot_index_file(csv_path: Path) -> Path:
    ...
    ax.set_title(f"{index_name} Seasonal Movement by Year")
    ax.set_xlabel("Month")
    ...
    output_path = PLOTS_DIR / f"{csv_path.stem}_seasonality.png"
